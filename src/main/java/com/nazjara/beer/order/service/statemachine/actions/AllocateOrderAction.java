...
import com.nazjara.beer.order.service.model.events.AllocateOrderRequest;
...
import static com.nazjara.beer.order.service.config.JmsConfig.ALLOCATE_ORDER_QUEUE;
...
public class AllocateOrderAction implements Action<BeerOrderStatusEnum, BeerOrderEventEnum> {
    ...
    public void execute(StateContext<BeerOrderStatusEnum, BeerOrderEventEnum> context) {
        ...
        jmsTemplate.convertAndSend(ALLOCATE_ORDER_QUEUE, AllocateOrderRequest.builder()
                .beerOrder(beerOrderMapper.beerOrderToDto(beerOrder))
                .build());

        log.debug("Sent allocation request to the queue for order id: " + beerOrderId);
    }
}
